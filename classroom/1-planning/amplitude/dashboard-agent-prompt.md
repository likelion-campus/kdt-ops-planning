...
## 사용 이벤트 (snake_case · Notion 최종본 13종)
- kdt_classroom_home_entered
- kdt_class_quiz_started / kdt_class_quiz_submitted (correct_count, score, duration_sec)
- kdt_class_quiz_explanation_viewed (결과 화면 열람) / kdt_wrong_answer_note_viewed
- kdt_supplementary_quiz_started (set_question_count, selected_date_count, set_wrong_answer, daily_set_count, daily_limit_reached) / kdt_supplementary_quiz_submitted (set_question_count, correct_count, daily_set_count, daily_limit_reached)
- kdt_til_submitted (char_count, write_duration_sec) / kdt_til_updated
- kdt_project_submitted (project_round_type) — 회차 마일스톤 / kdt_project_viewed / kdt_project_published_viewed (project_round_url)
...
- 정의: 같은 날(day) 안에서 kdt_class_quiz_submitted AND kdt_til_submitted
...
- TIL 빈껍데기율 = kdt_til_submitted 중 char_count/write_duration_sec 하위 임계값 비율. (has_external_paste 속성 제거됨)
